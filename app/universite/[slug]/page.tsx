...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${university} Bölümleri, 2026 Sıralamaları ve Kontenjanları`;
  const description = `${university} için ${programs.length} programı; 2026 başarı sıralamaları, 2025 karşılaştırması, 2026 kontenjanları, URAP ${first?.urap ? `#${first.urap}` : "verisi"}, THE ve QS göstergeleriyle inceleyin.`;
  return {
    title,
    description,
    ...
    <>
      ...
      <ProgramSearch
        ...
        hero={{
          ...
          description: `Önce üniversitenin akademik profilini, sıralamalarını ve araştırma göstergelerini incele; ardından tüm bölümleri 2026–2025 başarı sıralamaları ve 2024–2026 kontenjanlarıyla karşılaştır.${researchSummary}`,
        }}
        universityProfile={{
          name: university,
          city: first.city,
