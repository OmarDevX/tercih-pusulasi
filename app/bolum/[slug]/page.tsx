...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const bestRank = programs.find((program) => program.rank2025 !== null)?.rank2025 ?? null;
  const title = `${group.name} 2025 Taban Sıralamaları ve 2026 Kontenjanları`;
  const description = `${group.name} için ${programs.length} programı; 2025 son yerleşen sıralamaları${bestRank ? ` ${formatNumber(bestRank)}'den başlıyor` : ""}, 2024 karşılaştırması, 2026 kontenjanları, URAP, THE ve QS verileriyle inceleyin.`;
  return {
    title,
    description,
