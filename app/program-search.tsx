...
type SortField = "rank2025" | "urap" | "the" | "qs";
...
const SORT_LABELS: Record<SortOption, string> = {
  "rank2025-asc": "2025 başarı sırası: düşük → yüksek",
  "rank2025-desc": "2025 başarı sırası: yüksek → düşük",
...
const programRankingValue = (program: Program, field: SortField) => {
  if (field === "rank2025") return program.rank2025;
...
const fetchProgramChunk = async (selection: SearchSelection, page: number) => {
  const params = new URLSearchParams({ page: String(page), data: "2026-quota-v1" });
...
export default function ProgramSearch({
  ...
  const [sortOption, setSortOption] = useState<SortOption>("rank2025-asc");
  ...
  useEffect(() => {
    if (initialPrograms.length > 0 && initialUniversities.length > 0) return;
    ...
    fetchPrograms(initialSelection ?? { kind: "subject", group: DEFAULT_GROUP })
      .then((rows) => {
  ...
  }, [initialPrograms.length, initialSelection, initialUniversities.length]);
  ...
  const filteredPrograms = useMemo(() => {
    ...
    return programs
      ...
      .filter((program) => {
        if (program.rank2025 === null) return min === null && max === null;
        if (min !== null && program.rank2025 < min) return false;
        if (max !== null && program.rank2025 > max) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  const resolvedHero: CatalogHeroCopy = hero ?? {
    ...
    description:
      "2024–2025 YKS son yerleşen sıralamaları, 2024–2026 kontenjanları, URAP 2025–2026, THE 2026 ve QS 2027 tek ekranda.",
  };
        ...
        <div className="shell">
          ...
          <div className={`filter-panel ${mobileFiltersOpen ? "open" : ""}`}>
            ...
            <label htmlFor="min-rank">
              <span>2025 sıra alt sınırı</span>
              <input
                id="min-rank"
                type="number"
                inputMode="numeric"
            ...
            <label htmlFor="max-rank">
              <span>2025 sıra üst sınırı</span>
              <input
                id="max-rank"
                type="number"
                inputMode="numeric"
            ...
            {catalogError ? (
              ...
              visiblePrograms.map((program, programIndex) => {
                ...
                const improved =
                  ...
                  program.rank2024 !== null &&
                  program.rank2025 < program.rank2024;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="metric rank-metric">
                      ...
                      {program.rank2024 !== null && program.rank2025 !== null && (
                    ...
                    <div className="metric">
                      <span>2024 son yerleşen</span>
                      <strong>{formatRank(program.rank2024)}</strong>
                    </div>
          ...
          <section className="source-section" aria-labelledby="sources-title">
            ...
            <div className="source-grid">
              <a href="https://yokatlas.yok.gov.tr/" target="_blank" rel="noreferrer">
                <span>YKS</span>
                <strong>2024–2025 son yerleşen</strong>
                <p>YÖK Atlas tercih kılavuzu kataloğundaki tüm güncel program seçenekleri ve geçmiş sıralamalar.</p>
              </a>
