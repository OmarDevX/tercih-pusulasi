...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!program) return {};
  const title = `${program.university} ${program.programName}: 2025 Sıralaması`;
  const description = `${program.programName} programının 2025 son yerleşen sırası ${formatNumber(program.rank2025)}, 2024 sırası ${formatNumber(program.rank2024)}, 2026 kontenjanı ${formatNumber(program.quota2026)}. ${program.university}, ${program.city}.`;
  return {
    title,
    description,
