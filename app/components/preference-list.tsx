...
const isStoredProgram = (value: unknown): value is Program => {
  if (!value || typeof value !== "object") return false;
  const candidate = value as Partial<Program>;
  return typeof candidate.id === "string" && typeof candidate.programName === "string";
};
    ...
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        if (saved) {
          ...
          if (Array.isArray(parsed)) setSelectedPrograms(parsed.filter(isStoredProgram));
        }
      } catch {
                ...
                selectedPrograms.map((program, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <small>2025: {formatRank(program.rank2025)} · {program.city}{academicScores?.[program.university] !== undefined ? ` · Akademik puan ${formatAcademicScore(academicScores[program.university])}` : ""}</small>
                    </div>
